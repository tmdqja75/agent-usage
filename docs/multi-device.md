...
The profile repository's GitHub Actions workflow validates the public device/day records, merges their aggregates, and refreshes the managed README and SVG assets. The template lives in this collector repository at `templates/github-workflow.yml`; copy it into the profile repository only when profile initialization has been explicitly approved.
